    ...
    public class RefereeUI {
        ...
        private void drawManaCurve()
        {
            for (int p = 0; p < 2; p++)
            {
                ...
                bool overflow = maxmc * ConstantsUI.MC_GRAPH_STEP > ConstantsUI.MC_GRAPH_MAXSIZE;
        ...
        public void battle(int turn) {
            ...
            if (action != null && (action.type == Action.Type.ATTACK || action.type == Action.Type.USE)) {
                ...
                if (creature.isPresent()) {
                    ...
                    bool isOnBoard = graveyardCreature.isPresent();
                    if (!isOnBoard) {
        ...
        private bool actionPlayedOnSelf(Action action) {
            Card card = engine.state.cardIdMap.get(action.arg1);
        ...
        private bool actionDoesntAffectTargetCreature(Action action) {
            Card card = engine.state.cardIdMap.get(action.arg1);
